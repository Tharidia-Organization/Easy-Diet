...
import com.THproject.tharidia_easydiet.client.gui.DietaScreen;
...
public class DietaInventoryOverlay {
    ...
    /**
     * Renders the diet button above the armor/head slot column.
     */
    public static void renderFeatureButtons(GuiGraphics gui, int guiLeft, int guiTop, double mouseX, double mouseY, boolean mouseDown) {
        ItemStack dietIcon = new ItemStack(Items.COOKED_BEEF);
        
        SlotBounds bounds = getDietButtonBounds(guiLeft, guiTop);
        boolean hovered = bounds.contains((int) mouseX, (int) mouseY);
        boolean pressed = mouseDown && hovered;
        renderSlotButton(gui, bounds.x(), bounds.y(), bounds.size(), dietIcon, hovered, pressed);
    }
    ...
    public static void onMouseClicked(ScreenEvent.MouseButtonPressed.Pre event) {
        ...
        SlotBounds dietaButton = getDietButtonBounds(guiLeft, guiTop);
    ...
    private static SlotBounds getDietButtonBounds(int guiLeft, int guiTop) {
        int x = guiLeft + HEAD_SLOT_X_OFFSET;
