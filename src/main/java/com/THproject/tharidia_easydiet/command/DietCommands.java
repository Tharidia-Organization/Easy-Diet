        ...
        if (!profiles.isEmpty()) {
            for (ServerPlayer player : server.getPlayerList().getPlayers()) {
                net.neoforged.neoforge.network.PacketDistributor.sendToPlayer(
                    player, 
                    new com.THproject.tharidia_easydiet.network.DietProfileSyncPacket(profiles)
                );
            }
        }
    }
